...
const (
	...
	AuthorName  = "Luan Santos"
	AuthorEmail = "vim@luan.sh"
)
...
func init() {
	...
	flags.Usage = func() {
		fmt.Fprintf(os.Stderr, "boshtags version %s\n\n", Version)
...
func walkDir(names []string, dir string) ([]string, error) {
	e := filepath.Walk(dir, func(path string, finfo os.FileInfo, err error) error {
		...
		if strings.HasSuffix(path, ".yml") && !finfo.IsDir() {
...
func main() {
	...
	if len(outputFile) == 0 || outputFile == "-" {
		// For compatibility with older boshtags versions, also write to stdout
